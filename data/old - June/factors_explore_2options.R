setwd("/Users/kwalker/git_projects/yield/data/old - June")
...
fit1gs2 <- glm(formula = Final.Status ~ ACT + GPA +  
                   Royal15 + Total.Gift + Distance.Mhd + Zip.Median.Income + 
                   Zip.Alumni.Density + Music.Interest + Legacy.Relationship + 
                   Lutheran.Indicator + FAFSA, 
               family = "binomial", data = train1[, -2])
...
table(test1$Correct) # 92.0% correct / power = 0.952 / 0.031 alpha



################################ OPTION 2
data2 <- data[ , -c(8:14,20,34)]
data2fit <- data2[ ,-c(2)] # drop id
...
# fix binomial issues
for(j in c(4,7:11,21,22)) data2fit.imp[,j] <- round(data2fit.imp[,j])
...
train2 <- data2fit.imp[data2fit.imp$Year!="2016 Fall", ]
test2 <- data2fit.imp[data2fit.imp$Year=="2016 Fall", ]
...
table(test2$Correct) # 88.0% correct / power = .9255 / alpha = .0449
